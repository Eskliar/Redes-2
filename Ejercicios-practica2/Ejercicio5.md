...
La dirección 65.0.0.0 está en decimal con puntos (dotted decimal), es la forma normal en la que escribimos direcciones IPv4.
El “/8” dice que hay 8 bits de red y 24 bits de host (en IPv4 hay 32 bits).
En binario la red es:
...
Prefijo nuevo = 8 + 10 = /18
**Máscara /18 en decimal: 255.255.192.0**

1111 1111.|**1111 1111.11**|00 0000.0000 0000 -> máscara utilizada para identificar la red subneteada.

 Podemos escribir su equivalente en **notación CIDR**, le sumamos 10 al prefijo original (/8) ya que, en este caso, se toman 10 bits adicionales para subnetting. -> 65.0.0.0/18
...
65.204.64.0/18 -> **Dirección de subred** en *decimal*.
...
0100 0001.1100 1100.0111 1111.1111 1111 /18 -> **Dirección de broadcast de la red**.

65.204.127.255/18 -> **Dirección de broadcast en decimal**.
...
O *en binario* sumando uno (1) en la parte de bits asignados para host:
...
Ahora debemos identificar la **última dirección de red asignable**, tomando como referencia la dirección de broadcast de la subred. Al igual que la dirección de red, esta direccíon tampoco puede ser asignada para un host, ya que corresponde a la dirección de *broadcast* por lo que la última red asignable será **la anterior a esta.**

65.204.127.255/18 -> Dirección de broadcast.

65.204.127.254/18 -> **Última dirección de red asignable para host.**

O en binario, restando uno (1) en la parte de bits asignados para host:
